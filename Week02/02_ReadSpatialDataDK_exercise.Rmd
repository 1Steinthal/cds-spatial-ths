...
```{r}
names <- c("forests", "parks", "playgrounds", "shelters")
feature_count <- c(length(forests), length(parks), length(playgrounds), length(shelters))
feature_type <- c("Multipolygon", "Multipolygon", "point", "point")

meta <- rbind(names,feature_count, feature_type)
meta
```
...
# Load the sf + dplyr package
library(sf, dplyr, tidyverse)

# Use filter() to limit to over5ha parks and count
parks %>% 
...
big_forests <- forests %>% filter(unclass(st_area(forests)) > 200000)
...
plot(forests_geo, col = "green");plot(parks$geometry, col = "blue", add = T)
...
```{r}
dim_ele <- c(ncol(elevation), nrow(elevation))
...
Well, there are 5 bands...

By plotting one can suspect that: 
Band 1 & 2 & 3 & 4: Buildings?
Band 5: NA...
```{r}
nlayers(aarhus)
plot(aarhus)
...
Well, since one of them are a TIF, they have several bands for each colour (RGB), also the resolution is far bigger. A quick calculation lets us know that there are 32 times as many cells in the TIF than in the elevation sheet (taking into account the 5 bands as unique cells. Because TIF is a format for graphics it make sense that it is larger)


```{r}
ncell(aarhus)/ncell(elevation)
```
...
plot(elevation_crs, add = T);plotRGB(aarhus, stretch = "lin", add = T)
...
plot(elevation)
plot(elevation);plot(municipalities, add=T)
```
