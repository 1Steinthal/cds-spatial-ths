...
monuments <- st_transform(monuments, crs = 25832)
...
ggplot() +
  ...
  geom_sf(data = monuments, aes(size = anlnr))
```
...
1. What is the class of the object produced via `st_intersects()`? It is a list
2. Why do we use `parks[cont,]` to view the fully contained parks in the buffer plot? I guess it is the same...
3. How many parks and viking monuments respectively are there within the buffer? 79 monuments and 32 parks
...
# Plot the results
plot(nb_buf$geometry, border = "blue")
plot(st_geometry(ten_nearst), col = "red", pch = 19, add = TRUE)
```
...
4. What monument type did you choose to depict and what is its density per square km in Aarhus municipality? s
